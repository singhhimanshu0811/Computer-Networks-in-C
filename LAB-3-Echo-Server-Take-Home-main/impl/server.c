...
void findReply(char* msg, char* reply){
    ...
    char *token_list[1000]; int token_count=0;
    char* token;
    token=strtok(msg, delim);
    strcpy(token_list[token_count], token);
    token_count++;
    if(strcmp(token, "ECHO")!=0){
        ...
        int msg_length=strlen(token_list[1]);
        int given_length=atoi(token_list[2]);
        if(given_length<0){
            sprintf(reply, "%s", "Error: Negative number of bytes");
        }

        else if(msg_length< given_length){
            char msg1[10000]="OHCE:";
            char *str_len;
            sprintf(str_len, "%d", given_length);
            char msg4[50]="bytes sent)";
