...
void deser_test(struct packet* p, struct packet* p2)
{
    ...
    if(p->version != p2->version)
        printf("Deserialization Test1 Failed\n");
    else if(p->headerLength != p2->headerLength)
        printf("Deserialization Test2 Failed\n");
    else if(p->totalLength != p2->totalLength)
        printf("Deserialization Test3 Failed\n");
    else if(p->srcDept != p2->srcDept)
        printf("Deserialization Test4 Failed\n");
    else if(p->destDept != p2->destDept){
        printf("%d~%d", p->destDept, p2->destDept);
        printf("Deserialization Test5 Failed\n");
    }
    else if(p->checkSum != p2->checkSum)
        printf("Deserialization Test6 Failed\n");
    else if(p->hops != p2->hops)
        printf("Deserialization Test7 Failed\n");
    else if(p->type != p2->type)
        printf("Deserialization Test8 Failed\n");
    else if(p->ACK != p2->ACK)
        printf("Deserialization Test9 Failed\n");
    else if(p->headerLength == 6 && p->srcCampus != p2->srcCampus)
        printf("Deserialization Test10 Failed\n");
    else if(p->headerLength == 6 && p->destCampus != p2->destCampus)
        printf("Deserialization Test11 Failed\n");
    else if(strcmp(p->data, p2->data)){
        printf("%s~%s~%zu~%zu~%c\n", p->data, p2->data, strlen(p->data), strlen(p2->data), p2->data[strlen(p2->data)-1]);
        printf("Deserialization Test12 Failed\n");
    }
    else
